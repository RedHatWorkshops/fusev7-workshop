...
.. In the *Username* field, enter `jboss` as authorized user account.
.. In the *Password* field, enter `jboss` as the password.
...
+
----
todo-fuse.fuse-<$GUID>.apps.dev37.openshift.opentlc.com/swagger.json
----
+
...
+
----
https://todo-app.fuse-<$GUID>.apps.<$Environment_ID>.openshift.opentlc.com
----
+
...
+
[TIP]
For instance, the URL to a particular instance of a *To Do app* could be `https://todo-app.fuse-3824.apps.ap2.openshift.opentlc.com/` where `3824` is the *$GUID* and `ap2` is the *$Environment_ID*
+
